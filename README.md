...
- Người chơi tự tạo tài khoản thường. Tài khoản VIP chỉ admin mới tạo/cấp được. Mật khẩu tài khoản giới hạn từ 4 đến 12 ký tự.
- Admin tạo/xóa tài khoản, cấp/gỡ VIP, xem IP gần nhất, xem lịch sử đấu của từng tài khoản, mở khóa tài khoản, đặt lại mật khẩu và chỉnh chuỗi thắng trên bảng xếp hạng.
- Lưu tên hiển thị, avatar, 10 ván gần nhất và chuỗi thắng cho tài khoản vào file riêng `data/accounts.json`.
...
- Đã xóa hoàn toàn phần thay nền để file tài khoản nhẹ hơn.
...
File này chứa tài khoản, mật khẩu đã mã hóa, tên hiển thị, avatar, VIP/admin, trạng thái khóa, IP gần nhất, 10 ván gần nhất, lịch sử đấu và chuỗi thắng.
...
File backup chứa tài khoản, mật khẩu đã mã hóa, avatar, VIP/admin, trạng thái khóa, IP gần nhất, lịch sử đấu và chuỗi thắng. Không upload file backup công khai lên GitHub vì file có dữ liệu tài khoản, IP và session đăng nhập.
...
## Bản vá an toàn bộ nhớ / dữ liệu

Bản này giảm nguy cơ lỗi `JavaScript heap out of memory` bằng cách:

- Giới hạn dung lượng ảnh avatar còn tối đa 256KB.
- Giới hạn buffer Socket.IO còn 3MB để tránh upload payload quá lớn.
- Giữ lịch sử đấu mỗi tài khoản tối đa 50 ván.
- Giữ battle log/admin log tối đa 300 dòng.
- Tự tạo `data/accounts.autobak.json` trước mỗi lần ghi đè `data/accounts.json`.
- Nếu `accounts.json` lỗi/trống, server sẽ thử đọc `accounts.autobak.json` trước khi tạo dữ liệu mới.
- Chặn ghi đè `accounts.json` nếu dữ liệu tài khoản bất thường hoặc không còn tài khoản admin.

Lưu ý: nếu dùng Render Free không có Persistent Disk/database, dữ liệu trong thư mục `data` vẫn có thể mất khi service restart/redeploy. Nên tải backup tài khoản thường xuyên hoặc chuyển sang Persistent Disk/Supabase/PostgreSQL.

- Chỉ tài khoản VIP/Admin mới có quyền thay đổi avatar; tài khoản thường chỉ đổi được tên hiển thị.

- Mật khẩu người dùng được lưu bằng mã hóa một chiều, admin không xem lại được mật khẩu thật; admin chỉ có quyền đặt lại mật khẩu mới cho tài khoản.


## Giữ dữ liệu tài khoản không bị mất trên Render

Bản này lưu tài khoản trong file `accounts.json`. Để file không mất sau khi Render restart/redeploy, nên dùng Persistent Disk hoặc database.

Cách dùng Persistent Disk trên Render:

1. Vào Render project → Settings → Disks.
2. Tạo disk và đặt mount path, ví dụ: `/var/data`.
3. Vào Environment, thêm biến:
...
Sau đó server sẽ lưu `accounts.json`, `accounts.autobak.json` và `db.json` trong `/var/data` thay vì trong thư mục code.
